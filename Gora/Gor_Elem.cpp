...
String GorElemTypeName[9] = {"???", "��", "���", "�������", "���.���", "���.����", "�������1", "�������2", "��2"};
...
void tG_Strel_Y::CalcPRRTDS()
{
    rr = sh_y * 2;//MUL_Y/3;
    int dx0 = 3 * MUL_X;   // ������ �� ������ �����
    int dx1 = rr * 3 / 4;//MUL_Y*2/4; // ������ �� �����
    int dx2 = 1 * MUL_X;//MUL_Y;   // ����� �����
    int dx22 = rr;//1*MUL_X/2;//MUL_Y;   // ����� ������������
    int shpr = sh_y / 2;   // 1/2 ������ �����
    int shpr2 = dx22 / 2 - shpr;   // ������ ������ ������������
    int prd = 4; // ������ �� �����
    int n = 1;
...
//                 )
{
    ...
    if (mkRRC == 33) {
        ...
        if ((A == 1)) clrpPP = GELT;
        if ((A != 1) && (mkRRC == 1)) clrpPP = COLOR_GOR_A; // ���������� ��� RRC=1
...
void tG_ZAMI::ShowZAM(float stupen)
{
    ...
    */
    ...
    for (int d = 0; d < 2; d++) {
        for (int tr = 0; tr < kolTR - kolTRhalf; tr++) { // ������������ �����
                   ...
                   int maxRC)
{
    ...
    for (int i = 0; i < maxRC; i++){
        if (RC != NULL) {
            ...
            t_Descr* DD=GetDescr_imp1(RC->impuls_busi,       // �������������� ������ / ���� ������
                                      se,          // start=0/end=1
                                      end_slg_inc
            );
...
void tG_OTCLABEL::UpdateState()
{
   ...
   // int flen   = len;
    ...
    // ������� ������
    t_Descr* _Descr= GetDescr_imp(imp_descr_start,       // �������������� ������ / ���� ������
                       5,
                       0,//  se,          // start=0/end=1
                       false//end_slg_inc
                      );
    if (_Descr==NULL)  _Descr= GetDescr_imp(imp_descr_start,       // �������������� ������ / ���� ������
                       5,
                       1,//  se,          // start=0/end=1
                       false//end_slg_inc
                      );
    ...
    /*if (mar_f == -1) {
        if (!GetDescrData_imp(imp_descr_start,          // �������������� ������ ������ |    N �� �� � ��     |
                              5, 0,       // start
                              NTP,
    ...
    // ������� �����
    if (mar_f == -1) {
        if (!GetDescrData_imp(imp_descr_end,          // �������������� ������ ������ |    N �� �� � ��     |
                              5,  1,       // end
                              NTP,
...
void tG_TPLABEL::Show()
{
    ...
    if (masy == 0) {
        ...
        if ((VesBukv != 0) && (w <= 2) && (Stupen <= 0)) {
            ...
            if (VesBukv == 0) {
                switch (Stupen) {
                case 1: pictn=132; stVes="�"; break;
                case 2: pictn=133; stVes="��"; break;
                case 3: pictn=134; stVes="�"; break;
                case 4: pictn=135; stVes="�"; break;
                default:pictn=131; stVes="";
                }
            } else {
                switch (Stupen) {
                case 1: pictn=136; stVes="�"; break;
                case 2: pictn=137; stVes="��"; break;
                case 3: pictn=138; stVes="�"; break;
                case 4: pictn=139; stVes="�"; break;
                default:pictn=131; stVes="";
                }
            }
    ...
    if (AddrLK1Slot+addr1+AddrLK2Slot+addr2>0){
        ...
        if (G_ZAMI!=NULL){
           ...
           if ((_Descr==NULL)&&(G_ZAMI->NTP==3)) _Descr=FindDescr(G_ZAMI,
                ...
                if (_Descr==NULL)  _Descr= GetDescr_imp(imp_descr_end,       // �������������� ������ / ���� ������
                       5,
                       1,//  se,          // start=0/end=1
                       false//end_slg_inc
                      );
...
tG_KZP2::tG_KZP2()
        : tG_RC()
{
    szAll.cx = 120; szAll.cy = 14;// ������ ����� ��� ������ ����������� � ������ ���������
    xxNP = szAll.cx + 5;
    szNSV.cx = 24;  szNSV.cy = 14;// ������ ������ �����������
    szNP.cx = 32;   szNP.cy = 24;// ������ ������ ������ �����������
    szV.cx = 32;    szV.cy = 24;// ������ ������ ��������
    szNZ.cx = 32;   szNZ.cy = 20;// ������ ������ ������ ���������
    xxNZ = xxNP + szNP.cx + 5;
