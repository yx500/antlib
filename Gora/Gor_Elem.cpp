  ...
  for (int i = 0; i < stan->POLE.size(); ++i) {
    for (int j = 0; j < stan->POLE[i]->GetArraySize(); j++) {
      ...
      if (ac->GetType() == G_ZAMP) {
        ...
        if ((zamp->MinWay == olegwaynum) && (zamp->PutPark != 0))
          return zamp->PutPark;
      }
      if (ac->GetType() == G_KZP2) {
        tG_KZP2* kzp2 = dynamic_cast<tG_KZP2*>(ac);
        if (kzp2->mar == olegwaynum)
          return kzp2->PutPark;
      }
    }
  }
...
void tGRC0::UpdateState()
{
  ...
  int rimpuls_sv[2];
  for (int i = 0; i < 2; i++)
    rimpuls_sv[i] = f(impuls_sv[i]);
  for (int i = 0; i < 2; i++)
    StateChanged = StateChanged | (rimpuls_sv[i] != fimpuls_sv[i]);
  for (int i = 0; i < 2; i++)
    fimpuls_sv[i] = rimpuls_sv[i];
}
...
void tG_Strel_Y::Show()
{
  ...
  if (_is == P) {
    ...
    PolyColor(clBM, PM, fillclrM, 6, PW);
    ...
    setfillstyle(_styll1, clBP);
    PolyColor(clBP, PP, fillclrP, 10, PW);
  }
  /*  - */
  if (_is == M) {
  ...
  if (MOD == ED) {
    if (stNext[1][0] == "")
      ShowStik(YP.M[1].x, YP.M[1].y, 61);
    if (stNext[0][1] == "")
      ShowStik(YP.M[4].x, YP.M[4].y, 61);
    if (stNext[0][0] == "")
      ShowStik(YP.M[6].x, YP.M[6].y, 61);
    ...
    if (bShowNumberPosition) {
      ...
      int xn, yn;
      setcolor(GELT);
      _SetText(F_LITT, LEFT_TEXT, TOP_TEXT);
      _is = P;
      GetNumberPosition(xn, yn, 0, 0, 0);
      OutTextXY(xn, yn, "+N");
      fillellipse(xn, yn, 2, 2);
      _is = M;
      GetNumberPosition(xn, yn, 0, 0, 0);
      OutTextXY(xn, yn, "-N");
      fillellipse(xn, yn, 2, 2);
...
void tG_Strel_Y::GetNumberPosition(int& X, int& Y, int Width, int Height, int direct)
{
  ...
  if (Pol == P) {
    X = YP.M[0].x + NP[0].x * MUL_X / 10;
    Y = YP.M[0].y + NP[0].y * MUL_Y / 10;
  } else if (Pol == M) {
    X = YP.M[0].x + NP[1].x * MUL_X / 10;
    Y = YP.M[0].y + NP[1].y * MUL_Y / 10;
  } else {
    X = YP.M[0].x;
    Y = YP.M[0].y;
  }
  ...
  for (int m = 0; m < 2; m++) {
    ...
    for (int n = 0; n < StrelPrizak[m]; n++) {
      if ((rc_next != NULL) && (rc_next->fimpuls_busi == 1))
        return true;
      if (rc_next != NULL)
        rc_next = dynamic_cast<tGRC0*>(rc_next->pNext[0][0]);
    }
  }
  return false;
}
...
void tG_Strel_Y::UpdateState()
{
  ...
  for (int i = 0; i < 8; i++)
    StateChanged = StateChanged | (rimpuls_pr[i] != fimpuls_pr[i]);
  for (int i = 0; i < 8; i++)
    fimpuls_pr[i] = rimpuls_pr[i];
  ...
  if ((impuls_pr[i_prp] != 0) && (fimpuls_pr[i_prp] == 1)) {
    clrp[i_prp] = GELT;
    clrb[i_prp] = RED; /*clrp[i_sk]=clrp[i_prp];*/
  }
  if ((impuls_pr[i_prm] != 0) && (fimpuls_pr[i_prm] == 1)) {
    clrp[i_prm] = GELT;
    clrb[i_prm] = RED; /*clrp[i_sk]=clrp[i_prm];*/
  }
  ...
  if ((impuls_pr[i_ped1] != 0) && (fimpuls_pr[i_ped1] == 1)) {
    clrp[i_ipd] = GELT;
    clrb[i_ipd] = RED; /*clrp[i_sk]=clrp[i_prp];*/
  }
  ...
  if ((impuls_pr[i_uvk_block] != 0) && (fimpuls_pr[i_uvk_block] == 1)) {
    clrp[i_uvk_block] = RED;
    clrb[i_uvk_block] = RED;
  }
...
void tG_Strel_Y::ShowPR()
{
  ...
  TPoint Prm2[3] = {Td[1], Td[1], T[2]};
  Prm2[1].x -= DXS;
  TPoint Prm1[3] = {T[7], Td[2], Td[2]};
  Prm1[1].x += DXS;
  offset_points(DXS1, 0, Prm1, 3);
  offset_points(-DXS1, 0, Prm2, 3);
  TPoint Prp2[3] = {Td_[1], Td_[1], T_[2]};
  Prp2[1].x -= DXS;
  TPoint Prp1[3] = {T_[7], Td_[2], Td_[2]};
  Prp1[1].x += DXS;
  offset_points(DXS1, 0, Prp1, 3);
  offset_points(-DXS1, 0, Prp2, 3);
...
void ShowTR1(int x, int y, int clrp, int clrb, int napr, ttrchast trchast = _trfull)
{
  ...
  if (trchast == _trhalf)
    clrp = TXT; // Tr[2].x=Tr[0].x+sh_y/2*napr/2;
...
//                 )
{
  ...
  if (mkRRC == 33) {
    ...
    if ((A == 1))
      clrpPP = GELT;
    if ((A != 1) && (mkRRC == 1))
      clrpPP = COLOR_GOR_A; // обрамление при RRC=1
    if ((mkRRC != tsRRC) && (tsRRC != 33))
      clrpPP = RED;
    if (!bNomkErrCS)
      if (((impuls_zam[_impROSPUSK] == 0) || (fimpuls_zam[_impROSPUSK] == 1)) && (A == 1) && (mkErrCS == 1))
        clrbPP = KRA_ZEL;
  }
...
void tG_ZAMI::ShowZAM(float stupen)
{
  ...
  */
  ...
  if (stupen == -2) { //ситрание фоном
    ...
    ZTr[2] = TPoint(ZT[0][1].x + 2, ZTr[1].y);
    ZTr[3] = TPoint(ZT[0][1].x + 2, ZTr[0].y);
    PolyColor(FON, ZTr, clr1, 4);
    ZTr[0] = TPoint(ZT[1][0].x - 2, T[13].y + 1);
    ZTr[1] = TPoint(ZT[1][0].x - 2, ZTr[0].y + hpodl);
    ZTr[2] = TPoint(ZT[1][1].x + 2, ZTr[1].y);
    ZTr[3] = TPoint(ZT[1][1].x + 2, ZTr[0].y);
    PolyColor(FON, ZTr, clr1, 4);
    return;
  }

  if ((clrbPP != FON) || (clrpPP != FON)) {
    setfillstyle(SOLID_FILL, clrbPP);
    setcolor(clrpPP);
    ZTr[0] = TPoint(ZT[0][0].x - 2, T[0].y - 1);
    ZTr[1] = TPoint(ZT[0][0].x - 2, ZTr[0].y - hpodl);
    ZTr[2] = TPoint(ZT[0][1].x + 3, ZTr[1].y);
    ZTr[3] = TPoint(ZT[0][1].x + 2, ZTr[0].y);
    roundrect(ZTr[0].x, ZTr[0].y, ZTr[2].x, ZTr[2].y, sh_y, sh_y);
    ZTr[0] = TPoint(ZT[1][0].x - 2, T[13].y + 1);
    ZTr[1] = TPoint(ZT[1][0].x - 2, ZTr[0].y + hpodl);
    ZTr[2] = TPoint(ZT[1][1].x + 3, ZTr[1].y);
    ZTr[3] = TPoint(ZT[1][1].x + 2, ZTr[0].y);
    roundrect(ZTr[0].x, ZTr[0].y, ZTr[2].x, ZTr[2].y, sh_y, sh_y);
  }
  ...
  if ((TipStupen != 0) && (stupen > 0)) {
    if ((stupen - (int)stupen) > 0)
      kolTRhalf = 1;
    kolTR = (int)stupen + kolTRhalf;
  }
  if (kolTR > 0)
    dxTR = (ZT[0][1].x - ZT[0][0].x) / (kolTR + 1);
  ...
  for (int d = 0; d < 2; d++) {
    ...
    if (kolTRhalf > 0)
      ShowTR1(ZT[0][0].x + dxTR * (kolTR), yTR[d], clrpTR, clrbTR, naprTR[d], _trhalf);
  }
...
void tG_ZAMI::Show()
{
  ...
  // считаем ступень
  float stupen = 0;
  if (fimpuls_zam[_impR] == 1)
    stupen = -1;
  else if (fimpuls_zam[_impT4] == 1)
    stupen = 4;
  else if (fimpuls_zam[_impT3] == 1)
    stupen = 3;
  else if (fimpuls_zam[_impT2] == 1)
    stupen = 2;
  else if (fimpuls_zam[_impT1] == 1)
    stupen = 1;

  if (TipStupen != 0) {
    if (MOD == ED) {
      stupen = 0.4;
      if (impuls_zam[_impR] != 0)
        stupen += 1;
      if (impuls_zam[_impT1] != 0)
        stupen += 1;
      if (impuls_zam[_impT2] != 0)
        stupen += 1;
      if (impuls_zam[_impT3] != 0)
        stupen += 1;
    } else {
      if ((fimpuls_zam[_impT1] == 1) && (fimpuls_zam[_impT2] == 1))
        stupen = 0.5;
      else if ((fimpuls_zam[_impT1] == 1) && (fimpuls_zam[_impT3] == 1))
        stupen = 1.5;
      else if ((fimpuls_zam[_impT2] == 1) && (fimpuls_zam[_impT3] == 1))
        stupen = 2.5;
      if ((fimpuls_zam[_impT1] == 1) && (fimpuls_zam[_impT2] == 1) && (fimpuls_zam[_impT3] == 1))
        stupen = 3.5;
    }
  }
...
void tG_ZAMI::UpdateState()
{
  ...
  int rimpuls_zam[11];
  for (int i = 0; i < 11; i++)
    rimpuls_zam[i] = f(impuls_zam[i]);

  for (int i = 0; i < 11; i++)
    StateChanged = StateChanged | (rimpuls_zam[i] != fimpuls_zam[i]);

  for (int i = 0; i < 11; i++)
    fimpuls_zam[i] = rimpuls_zam[i];
  ...
  if ((IsClrMig(clrpTR)) || (IsClrMig(clrpTR)) || (IsClrMig(clrbTR)) ||
      (IsClrMig(clrpK)) || (IsClrMig(clrbK)) || (IsClrMig(clrpPP)) || (IsClrMig(clrbPP)))
    StateChanged = true;
}
...
void tG_ZKR::makebs(int dn)
{
  for (int i = 0; i < 3; i++) {
    ...
    Bs[i][0][1] = y - sh_y / 2 - 1;
    Bs[i][0][2] = xt;
    Bs[i][0][3] = Bs[i][0][1] - sh_y - 2;
    Bs[i][0][4] = xt + sh_y * dn;
    Bs[i][0][5] = Bs[i][0][1] - sh_y / 2 - 1;
    Bs[i][0][6] = Bs[i][0][0];
    Bs[i][0][7] = Bs[i][0][1];
    ...
    Bs[i][1][1] = y + sh_y / 2 + 1;
    Bs[i][1][2] = xt;
    Bs[i][1][3] = Bs[i][1][1] + sh_y + 2;
    Bs[i][1][4] = xt + sh_y * dn;
    Bs[i][1][5] = Bs[i][1][1] + sh_y / 2 + 1;
    Bs[i][1][6] = Bs[i][1][0];
    Bs[i][1][7] = Bs[i][1][1];
  }
  int xt2[6];
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if ((i == 0) || (i == 2) || (i == 4)) {
      Bs2[i][0][0] = xt;
      Bs2[i][0][1] = y - sh_y / 2 - 1;
      Bs2[i][0][2] = xt - sh_y / 2;
      Bs2[i][0][3] = Bs2[i][0][1] - sh_y - 2;
      Bs2[i][0][4] = xt + sh_y / 2;
      Bs2[i][0][5] = Bs2[i][0][1] - sh_y - 2;
      Bs2[i][0][6] = Bs2[i][0][0];
      Bs2[i][0][7] = Bs2[i][0][1];
    } else {
      Bs2[i][0][0] = xt;
...
void tG_ZKR::Show()
{
  ...
  if (MOD == ED) {
    for (int i = 0; i < 3; i++)
      if (impuls_zkr[i_D1 + i] != 0)
        clrD[i] = TXT;
      else
        clrD[i] = FON;
    for (int i = 0; i < 6; i++)
      if (impuls_zkr[i_D11 + i] != 0)
        clrD2[i] = TXT;
      else
        clrD2[i] = FON;
    for (int i = 0; i < 2; i++)
      if (impuls_zkr[i_rtds1 + i] != 0)
        clrRTDS[i] = TXT;
      else
        clrRTDS[i] = FON;
  } else {
    for (int i = 0; i < 3; i++) {
      if (impuls_zkr[i_D1 + i] == 0)
        continue;
      if (fimpuls_zkr[i_D1 + i] == 1)
        clrD[i] = COLOR_B;
      else if (fimpuls_zkr[i_D1 + i] == 0)
        clrD[i] = FON1;
      else
        clrD[i] = BIRUZOVIJ;
    }
    for (int i = 0; i < 2; i++) {
      if (fimpuls_zkr[i_rtds1 + i] == 0)
        clrRTDS[i] = FON1;
      else if (fimpuls_zkr[i_rtds1 + i] == 1)
        clrRTDS[i] = COLOR_B;
      else
        clrRTDS[i] = BIRUZOVIJ;
    }
    for (int i = 0; i < 6; i++) {
      if (impuls_zkr[i_D11 + i] == 0)
        continue;
      if (fimpuls_zkr[i_D11 + i] == 1)
        clrD2[i] = TXT;
      else if (fimpuls_zkr[i_D11 + i] == 0)
        clrD2[i] = FON1;
      else
        clrD2[i] = BIRUZOVIJ;
    }
  ...
  if (params_zkr[_zkr_tip_vivod_dso] == 0) {
    ...
    for (int i = 0; i < 3; i++) {
      if (impuls_zkr[i_D1 + i] == 0)
        continue;
      dn = d;
      if ((fimpuls_zkr[i_D1 + i] == 1) && (fimpuls_zkr[i_DN1 + i] == 0))
        dn = -d;
...
void tG_ZKR::UpdateState()
{
  ...
  for (int i = 0; i < _zkr_max_imp; i++)
    rimpuls_zkr[i] = f(impuls_zkr[i]);
  for (int i = 0; i < _zkr_max_imp; i++)
    StateChanged = StateChanged | (rimpuls_zkr[i] != fimpuls_zkr[i]);
  for (int i = 0; i < _zkr_max_imp; i++)
    fimpuls_zkr[i] = rimpuls_zkr[i];
}
...
void tG_OTCLABEL::UpdateState()
{
  ...
  if (_Descr == NULL)
    _Descr = GetDescr_imp(imp_descr_start, // Местоположение Головы / жопы отцепа
                          5,
                          1,    //  se,          // start=0/end=1
                          false // end_slg_inc
    );
  ...
  */
  if (_Descr != NULL) {
    if (memcmp(&Descr, _Descr, sizeof(Descr)) != 0)
      StateChanged = true;
    ...
    if ((VesBukv != 0) && (w <= 2) && (Stupen <= 0)) {
      ...
      if ((!bShowVnadv) && (p_rzp > 0) && (NTP <= 2)) {
        ...
        if (p_rzp == 4) { //  4 - скорость не откорректирована
          fontcol = C_D;
        } else if (p_rzp <= 3) { //  1,2,3 - скорость откорретирована
          fontcol = COLOR_PSV;
        }
        setcolor(fontcol);
      }
  ...
  if (AddrLK1Slot + addr1 + AddrLK2Slot + addr2 > 0) {
    ...
    if (rc_busy > 0) {
      ...
      for (int i = 0; i < rc_busy; i++) {
        if (RC == NULL)
          break;
        if ((RC->impuls_busi != 0) && (RC->fimpuls_busi == 1)) {
          bsh = true;
          break;
        }
    ...
    fimp_krasn = rimp_krasn;
    fimp_krasn_mig = rimp_krasn_mig;
    ...
    if (G_ZAMI != NULL) {
      ...
      if (_Descr == NULL)
        _Descr = GetDescr_imp(imp_descr_end, // Местоположение Головы / жопы отцепа
                              5,
                              1,    //  se,          // start=0/end=1
                              false // end_slg_inc
        );
  ...
  if (ADK3 != "") {
    ...
    if (ADK3Cell != NULL) {
      if (ADK3 == "a") {
        ...
        FADK3_val.printf("%2i", ADK3Cell->r);
    } else {
      FADK3_val = "";
    }
    if (_FADK3_val != FADK3_val)
      StateChanged = true;
  }
  if (bShowVnadv) {
    V_Zad = 0;
...
void tG_KZP::Show()
{
  ...
  if (W > 0) {
    ...
    if (xx >= 0) {
      if (Direct == 0) {
        T[0] = TPoint(Tpr.x + xx, T[0].y);
        T[3] = TPoint(Tpr.x + xx, T[3].y);
      } else {
        T[1] = TPoint(Tpr.x + xx, T[1].y);
        T[2] = TPoint(Tpr.x + xx, T[2].y);
      }
    ...
    if (MOD == ED) {
      ...
      if ((bShowOs)) {
        ...
        if (tip == 21) {
          String stV;
          if (outParam == 0)
            stV = IntToStr(OsyCell_21.V);
          if (outParam == 1)
            stV = IntToStr(OsyCell_21.D);
          if (outParam == 2)
            stV = IntToStr(OsyCell_21.E);
          if (outParam == 3)
            stV = IntToStr(OsyCell_21.EV);
          if (outParam == 4)
...
{
  szAll.cx = 120;
  szAll.cy = 14; // размер всего без текста свободности и текста занятости
  xxNP = szAll.cx + 5;
  szNSV.cx = 24;
  szNSV.cy = 14; // размер текста свободности
  szNP.cx = 32;
  szNP.cy = 24; // размер текста текста свободности
  szV.cx = 32;
  szV.cy = 24; // размер ячейки скорости
  szNZ.cx = 32;
  szNZ.cy = 20; // размер текста текста занятости
  xxNZ = xxNP + szNP.cx + 5;
...
void tG_KZP2::UpdateState()
{
  ...
  if (pKvKzp != NULL) {
    if (memcmp(&KvKzp, pKvKzp, sizeof(KvKzp)) != 0)
      StateChanged = true;
  ...
  if (pKzpInf != NULL) {
    if (memcmp(&KzpInf, pKzpInf, sizeof(KzpInf)) != 0)
      StateChanged = true;
  ...
  if (outD == 1) {
    ...
    if (GetKZPData(AddrKZP - 1, KzpD, KzpE, KzpV, KzpP)) {
      if ((KvKzp.val != KzpD) || (KvKzp.stat != KzpE) || (KvKzp.val_ks != KzpV) || (KvKzp.val_ks_i != KzpP))
        StateChanged = true;
      KvKzp.val = KzpD;
      // KvKzp.stat=KzpE;
      if (KzpE == 65535)
...
double GetVkzp(t_KzpInf* KzpInf, int dxV)
{
  ...
  if ((KzpInf->points_count < 2) || (KzpInf->points_count > 16))
    return KzpInf->v;
  ...
  tim1 = KzpInf->points[i].tim;
  val1 = KzpInf->points[i].val;
  do {
  ...
  if (dtim == 0)
    return 0;
  if (KzpInf->points[KzpInf->points_count - 1].val < 30)
    return 0;
  if ((dtim != 0) && (dl >= dxV))
    v = (1. * dl / dtim * 1000) * 36.;
  return v;
...
void tG_KZP2::Show()
{
  ...
  if (Direct == 1) {
    ...
    FlipPx(TZ, 4, fx);
    FlipPx(TNP, 4, fx);
    FlipPx(TNP, 4, TNP[0].x, TNP[1].x - TNP[0].x);
    FlipPx(TNZ, 4, fx);
    FlipPx(TUS, 4, fx);
    FlipPx(TV, 4, fx);
    FlipPx(TV, 4, TV[0].x, TV[1].x - TV[0].x);
    svob_tj = RIGHT_TEXT;
  }
