...
int  Poligon::Close()
{
    ...
    Col_ST = 0;
    for (int i = 0; i < 30; i++) {
        if (ST[i] == STB[i]) STB[i] = NULL;

        if (ST[i]) delete ST[i];
        if (STB[i]) delete STB[i];
        ST[i] = STB[i] = NULL;
    }
    return 1;
}
...
int  Poligon::LoadYCH(const char *filename)
{
    ...
    for (CurrentStation = 0; CurrentStation < Col_ST; CurrentStation++) {
        ...
        } else STB[CurrentStation]->Open(fnm.c_str());
    }
    fclose(file);
...
int  Poligon::LoadYCE(const char *filename)
{
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        if (!FI->SectionExists(SectName)) continue;
...
void Poligon::Show()
{
    int i = CurrentStation;
    for (CurrentStation = 0; CurrentStation < Col_ST; CurrentStation++) {
        if (CurrentStation == i) Current_Name = 1;
        else Current_Name = 0;
        if (ST[CurrentStation!=NULL]) {
...
{ int i=CurrentStation;
  for(CurrentStation=0;CurrentStation<Col_ST;CurrentStation++){
      ...
      if(ST[CurrentStation!=NULL])ST[CurrentStation]->Zon();
  }
  CurrentStation=i;
}
*/
...
void Poligon::Del(int Nomer)
{
    ...
    for (CurrentStation = Nomer; CurrentStation < Col_ST; CurrentStation++) {
        if (ST[CurrentStation!=NULL])ST[CurrentStation] = ST[CurrentStation+1];
    }
    CurrentStation = i;
}
...
void Poligon::Go()
{
    ...
    if (CurrentPicture == BG) {
        ...
        for (CurrentStation = 0; CurrentStation < Col_ST; CurrentStation++) {
            ...
            if (ST[CurrentStation] != NULL)ST[CurrentStation]->Go();
        }
        //for(int s=0;s<Col_ST;s++)
        ...
        if (ST[CurrentStation] != NULL)STB[CurrentStation]->Go();
...
void Poligon::ShowActives()
{
    for (int i = 0; i < Col_ST; i++)
        if (ST[i])ST[i]->ShowActives();
}

void Poligon::Clear()
{
    for (int i = 0; i < Col_ST; i++) {
        if (ST[i]) ST[i]->Clear();  if (STB[i])STB[i]->Clear();
    }
}
...
void Poligon::SaveYCH()
{
    ...
    if (file == NULL) Qui("Íå ìîãó îòêðûòü ôàéë ó÷àñòêà", 1, 0);
...
{
    static int ddd[256];
    for (int i = 0; i < ARRAYSIZE(ddd); i++)  ddd[i] = 2147483647;
    PAComp P = NULL;
    int sx, sy, mx, my;
    //char *name;
    for (int i = 0; i < Col_ST; i++) {
