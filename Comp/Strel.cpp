...
void Strel0::UpdateState()
{
  ...
  if (nomer < 0) {
    ...
    is = P;
    {
      ...
      if ((GetType() == SzT) || (GetType() == SzB)) {
        ...
        if ((fimpuls_otkl_plus == 1) && (fimpuls_mnus == 0))
          is = 2;
        if ((fimpuls_otkl_minus == 1) && (fimpuls_plus == 0))
          is = 1;
      }
      if (!is)
        is = 3;
    ...
    if ((_clr != clr) || (_is != is) || (_styll1 != styll1) || (_styll2 != styll2) || (_styll3 != styll3))
      StateChanged = true;
    _clr = clr;
    _is = is;
    _styll1 = styll1;
  ...
  if ((IsClrMig(clr)) || (IsClrMig(cbord1)) || (IsClrMig(cbord2)))
    StateChanged = true;
}
...
{
  if ((name[0] == 'Ч') && (name[1] == 'П'))
    return 0; // ЧЕТНЫЕ СТРЕЛКИ
  if ((name[0] == 'Н') && (name[1] == 'О'))
    return 0;
  if ((name[0] == 'Н') && (name[1] == 'П'))
    return 1; //  НЕЧЕТНЫЕ СТРЕЛКИ
  if ((name[0] == 'Ч') && (name[1] == 'О'))
    return 1;
  return 1;
}
...
void Strel0::SetUstPol(int pol)
{
  ...
  switch (pol) {
  case 1:
    SetStrelShowParams(-1, -1, 6, 1, -1, -1);
    break;
  case 2:
    SetStrelShowParams(-1, -1, 1, 6, -1, -1);
    break;
  case 3:
    SetStrelShowParams(-1, -1, 7, 6, -1, -1);
    break;
  ...
  case 5:
    ...
    break;
  case 6:
    SetStrelShowParams(-1, -1, 6, 6, -1, -1);
    break;

  case 7:
    SetStrelShowParams(-1, -1, 5, 1, -1, -1);
    break;
  case 8:
    SetStrelShowParams(-1, -1, 1, 5, -1, -1);
    break;
  case 9:
    SetStrelShowParams(-1, -1, 5, 5, -1, -1);
    break;

  default:
    SetStrelShowParams(-1, -1, -1, -1, -1, -1);
    break;
  }
}
...
void Info::Show()
{
  ...
  int is1 = clr;
  int is2 = impuls_busi;
  int is3 = styll1 | fimpuls_kmu;
  int is4 = styll2 | fimpuls_kzm;
...
void Napr::Show()
{
  ...
  if (MOD == RD) {
    ...
    if ((impuls_busi != 0) && (impuls_svob != 0) && ((fimpuls_busi + fimpuls_svob == 0) || (fimpuls_busi + fimpuls_svob == 2)))
      pclr = KRA_ZEL;
    if ((impuls_kzm != 0) && (impuls_svob2 != 0) && ((fimpuls_kzm + fimpuls_svob2 == 0) || (fimpuls_kzm + fimpuls_svob2 == 2)))
      pclr = KRA_ZEL;
  ...
  if (is == 5) {
    ...
    is = is % 3 + 1;
    // setcolor(BLUE_m);
    if ((impuls_busi == 0) && (impuls_kzm == 0) && (impuls_plus == 0) && (impuls_mnus == 0))
      clr = FON;
...
const char* P_Sve::GetName()
{
  ...
  if (_GetSigName_Func != NULL)
    _PerName = _GetSigName_Func(GetRealImp(impuls_busi));
  else
    _PerName = "?";
  return _PerName.c_str();
}
...
void P_Sve::UpdateState()
{
  ...
  default:
    ...
    if (masy == 1) {
      ...
      int fpit = 0;
      if ((pit1fid != 0) && (fpit1fid == 1))
        fpit = 1;
      if ((pit2fid != 0) && (fpit2fid == 1))
        fpit = 1;
      if ((lamp1A != 0) && (flamp1A == 1))
        fpit = 1;
      if ((lamp2A != 0) && (flamp2A == 1))
        fpit = 1;
      if ((lamp1B != 0) && (flamp1B == 1))
        fpit = 1;
      if ((lamp2B != 0) && (flamp2B == 1))
        fpit = 1;
...
const char* Bash::GetName()
{
  if (_GetSigName_Func != NULL) {
    if (impuls_busi > 1000)
      _BashName = _GetSigName_Func(GetRealImp(impuls_busi));
    else if (impuls_plus > 1000)
      _BashName = _GetSigName_Func(GetRealImp(impuls_plus));
  } else
    _BashName = "?";
...
void Bash::UpdateState()
{
  ...
  if ((i1 == 33) ||
    ...
    default:
      if (AO->TypRAMK == 1)
        fillcol = ColorBash2[i1 * 8 + i2 * 4 + i3 * 2 + i4];
      else
        fillcol = ColorBash[i1 * 8 + i2 * 4 + i3 * 2 + i4];
      break;
    case 10:
      if (ColorNum < 16)
        fillcol = UKSPSColors[ColorNum][1];
      else
        fillcol = BIRUZOVIJ;
      if (ColorNum < 16)
        pencol = UKSPSColors[ColorNum][0];
      else
        fillcol = C_D;
      break;
    case 11:
      if (ColorNum < 16)
        pencol = SAUTPenColors[ColorNum];
      else
        pencol = C_D;
      fillcol = FON;
      break;
    ...
    case 14:
      if (ColorNum < 16)
        pencol = KGUPenColors[ColorNum];
      else
        pencol = C_D;
      break;
    case 13:
      if (ColorNum < 16)
        pencol = DISKPenColors[ColorNum];
      else
        pencol = C_D;
      if (ColorNum < 16)
        fillcol = DISKFillColors[ColorNum];
      else
        fillcol = BIRUZOVIJ;
      break;
    case 15:
      if (fimpuls_busi == 33)
        ...
        if (ColorNum < 16)
          pencol = BASH15Colors[ColorNum][1];
        else
          pencol = C_D;
        if (ColorNum < 16)
          fillcol = BASH15Colors[ColorNum][0];
        else
          fillcol = BIRUZOVIJ;
      }
      break;
    ...
    case 17:
      if (ColorNum < 16)
        pencol = UTSStateColors[ColorNum][1];
      else
        pencol = C_D;
      if (ColorNum < 16)
        fillcol = UTSStateColors[ColorNum][0];
      else
        fillcol = BIRUZOVIJ;
      break;
    }
    ...
    if (CurrentPicture == BG) {
      ...
      default:
        break;
      case 10:
        if ((ColorNum < 16) && (UKSPSColors[ColorNum][2] == 0)) {
          fillcol = FON;
          pencol = FON;
        }
        break;
...
void Bash::Show()
{
  ...
  if (tip == 11) {
    ...
    switch (mas) {
    case 0:
      barx(x, y - sh_y / 2 - 1, x + sh_y * 2, y - 1.5 * sh_y - 1);
      break;
    case 1:
      barx(x, y + sh_y / 2 + 1, x + sh_y * 2, y + 1.5 * sh_y + 1);
      break;
    case 2:
      barx(x, y - sh_y / 2 - 2, x + sh_y * 2, y - 1.5 * sh_y - 2);
      break;
    case 3:
      barx(x, y + sh_y / 2 + 2, x + sh_y * 2, y + 1.5 * sh_y + 2);
      break;
  ...
  /*    КГУ      */
  if ((tip == 12) || (tip == 14)) {
    if ((MOD != ED) && (CurrentPicture == BG) && (fimpuls_kmu == 0) && (fimpuls_busi == 0))
      return;
    // if((CurrentPicture==BG)&&(fimpuls_kmu!=1)) return;
    int sx = mas * MUL_X / 3;
    int sy = masy * MUL_Y / 2;
...
{
  if ((prSTIK <= 0) || (CurrentPicture == BG))
    return; // в режиме участка только разрыв
  if (prSTIK == 0)
    return;
  setcolor(BLUE_m);
  int st_st = sh_y / 2 + 2;
  ...
  default:
    break;
  case 11:
    line(xx, yy - st_st, xx, yy + st_st + 1);
    break;
  case 12:
    line(xx, yy - st_st, xx, yy + st_st + 1);
