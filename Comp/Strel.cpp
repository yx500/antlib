...
static char _StrelPropName[24][20] = {
    ...
    "òñimpuls_block",   // 18
    "òñimpuls_predSA",  // 19
    "òñimpuls_predSApl",  // 20
    "òñimpuls_predSAmn",  // 21

    "òñimpuls_otkl_plus",// 22
    "òñimpuls_otkl_minus"// 23
...
void Strel0::UpdateState()
{
    ...
    // Âû÷èñëÿåì âñå ïàðàìåòðû ïðîðèñîâêè
    // is - ïîëîæåíèå
    // styll1 -  øòðèõîâêà çàïîëíåíèÿ ïî +
    // styll2 -  øòðèõîâêà çàïîëíåíèÿ ïî -
    // styll3 -?
    // clr - öâåò çàïîëíåíèÿ ïî ïîëîæåíèþ
    // cbord1;     // öâåò îêàíòîâêè +
    // cbord2;     // öâåò îêàíòîâêè -

    int priz_otse = 0;
    if (nomer < 0) priz_otse++;
    if (nomer < 0) {

        //öâåò çàëèâêè ïîëîæåíèÿ
        clr = LIN;
        ...
        is = P;
        {
            ...
            if ((GetType() == SzT) || (GetType() == SzB)) {
                ...
                // ïðè îòêëþ÷åíèè íàäî ñîõðàíèòü ïîëîæåíèå
        ...
        // øòðèõîâêà Styll
        /* styll1=1;styll2=1;styll3=1; - âíåøíèå ñòàâÿò
        if(
           (GetType()!=SzB)&&
           (GetType()!=SzT)&&
           (priz_otse==0)) TestMu();*/
    } else {
        // Ãîðëîâèííàÿ îáðàáîòêà
        //    TestMu();
...
int Def_gr(char *name)
{
    ...
    if ((name[0] == 'Í') && (name[1] == 'Î'))return 0;
    if ((name[0] == 'Í') && (name[1] == 'Ï'))return 1; //  ÍÅ×ÅÒÍÛÅ ÑÒÐÅËÊÈ
    if ((name[0] == '×') && (name[1] == 'Î'))return 1;
    return 1;
...
{
    // óñòàíàâëèâàåò èçâíå ïàðàìåòðû äëÿ ðèñîâêè ñòðåëêè
    // åñëè çíà÷åíèå<0 ñòðåëêà îïðåäåëÿåòñÿ ñàìà
    UpdateState();
    if (Aclr < 0) {
        //clr=LIN;
    } else {
...
{
    // óñòàíàâëèâåò ðåøåòêó íà ñòðåëêå â ïîëîæåíèå
...
void  Info::Show()
{
    ...
    int styll3 = 1;  //  << ÈÑÏÐÀÂÈË ÏÐÀÂÈËÜÍÎ ????
    clr = fimpuls_busi;
    if (nomer == -1) {
        ShowX(x, y); return;
    }
...
//     Super_Box(XX-60,YY_-10,XX+60,YY_+25,2,C_D,C_D,LIN,LIN);
    ...
    if ((CurrentPicture == LT) && (mas != 1)) {
        if (! AO->FLAG_POINT_REVERS)
            OutTextXY(XX, YY_ - 2,   "ÀÄÍ       ÀÄ×");
        else
            OutTextXY(XX, YY_ - 2,   "ÀÄ×       ÀÄÍ");
    }
    if (is2) {
        OutTextXY(XX, YY_ - 2, "Ð.Óïð");
...
{
     Strel::SetPropMap(m);
     impuls_svob2 = NewStrToOldImp(m.get("òñimpuls_svob2").c_str());
}
...
void  Napr::Show()
{
    ...
    if (MOD == RD) {
        ...
        // íåñîâïàäåíèå çàíÿòîñòè/ñâîáîäíîñòè
    ...
    default:
        ...
        if (masy == 1) {
            ...
            if ((cc >= 8) || (fimpuls_busi == 33)) {
             ...
             // ïåðååçä ÝÖ12
            int fzakr = fimpuls_busi;   //  çàêðûòèå
            int fap1 = fimpuls_plus;     //  àâàðèÿ 1
            int fizv = fimpuls_mnus;     //  èçâåùåíèå/çàêðûòèå
            int fispr = fimpuls_kzm;    //  èñïðàâíîñòü
            int fap2 = fimpuls_kmu;     //  àâàðèÿ 2
            clrl = C_D;
            clr = FON;
            if (fzakr == 33) {
                clr = BIRUZOVIJ;
...
//xx=x;
    ...
    switch (AO->TypPere) {
    case 32:
        //  Íó çíà÷èòü ýòî èçîáðàæåíèå ïåðåçäà - ñîãëàñíî ÎÑÒ32
        // Îñíîâûâàÿñü òîêà íà ñèãíàëàõ çàíÿòèÿ? àâàðèè è ÎÃÐÀÆÄÅÍÈß

        s = MUL_Y / 3;
        xx = x - MUL_X / 3;
        x2 = x + MUL_X / 3;
        //  ÇÄÅÑÜ ÑÄÅËÀÍÛ ÈÇÌÅÍÅÍÈß ÄËß 2Õ2
        if (AO->_2X2_)
            y3 = y - (10 + mas) * MUL_Y;
        else
            y3 = y - (5 + mas) * MUL_Y;
    ...
    case 321:
        ...
        //  Íó çíà÷èòü ýòî èçîáðàæåíèå ïåðåçäà - ñîãëàñíî ÎÑÒ32
        // íÎ áåç îãðàæäåíèÿ è ñ ó÷åòîì 3-õ ñèãíàëîâ !
...
{
    if ((fimpuls_kmu == 0) && (fimpuls_mu == 0)) //îòñå÷êà
...
{
    if ((prSTIK <= 0) || (CurrentPicture == BG)) return; // â ðåæèìå ó÷àñòêà òîëüêî ðàçðûâ
    if (prSTIK == 0) return;
    setcolor(BLUE_m);
    int st_st = sh_y / 2 + 2;
    switch (prSTIK) {
