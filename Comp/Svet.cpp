...
Svet_0::Svet_0()
{
    ...
    clrP = C_D;
    bGorl = false;
    Razmer[0] = 0; Razmer[1] = 0; Stek = 0;
    impuls_yach=fimpuls_yach=0;
    impuls_block=0;fimpuls_block=0;impuls_jelt=0;fimpuls_jelt=0;impuls_jeltzel=0;fimpuls_jeltzel=0;impuls_neispr2=0;fimpuls_neispr2=0;
    bZAKRKRA=false;
    yach_sdvig=0;
    bNiz=false;
    impuls_aum_a=0;
...
void  ShowKMUSvet(int x, int y, int iBusy, int iPlus, int iMinus, int mas, char * name, int _NAZVA_)
{
    ...
    line(x, y, x, y - MUL_Y);
    int Bs[8];
    Bs[0] = x;     Bs[1] = y   - MUL_Y * 1;
    Bs[2] = x + sh;  Bs[3] = y   - MUL_Y * 1;
    Bs[4] = x;     Bs[5] = y   - MUL_Y / 3;
    Bs[6] = x;     Bs[7] = y   - MUL_Y * 1;
    int ColorNum = iBusy * 2 + iPlus;
    if (ColorNum < 4) setfillstyle(1, KMUSvetColors[ColorNum]); else setfillstyle(1, BIRUZOVIJ);
    fillpoly(4, Bs);
    if ((MUL_Y > _NAZVA_) && (CurrentPicture != BG)) {
        ...
        int mm = mas; if (mm > 100)mm -= 100;
        switch (mm) {
        case 1:  _SetTextSize(-sh*2); OutTextXY(x, y, name); _SetTextSize(1); break;
        case 2:  break;
        default : OutTextXY(x, y, name);
        }
...
{
        if (CurrentPicture == BG) sh=sh*1.5; else
        sh=sh*2;
...
void  ShowPerSvet(int x, int y, int type, int iBusy, int iPlus, int iKzm, int mas, int masy, char * name,int fimpuls_block)
{
    ...
    int len;
    sh = sh_y / 2;
    if (masy == 2)y = y + MUL_Y - sh_y - 1; else if (masy == 3)y = y + MUL_Y + sh_y + 1;
    if (type == 4) len = sh_y;
    if (type == 3) len = -sh_y;
    ...
    if (iKzm == -1) { // рисуем фоном
        ...
        if (ColorNum < 4) brclr = PerSvetColors[ColorNum];
        if (iPlus==1) pclr=KRA_SER;
    }
    ...
    if (CurrentPicture == BG) {
        switch (mas) {
        default:showN=false; break;
        case 1:; break;
        case 2:showN=true; showsv=false; break;
        case 3:showsv=false; showN=false; break;
        }
    }
    if (showsv) {
        ...
        if ((CurrentPicture == BG) && (brclr != FON)) setcolor(brclr);
...
void Svet_0::UpdateState()
{
        ...
        if (impuls_busi + impuls_plus + impuls_mnus + impuls_kzm + impuls_kmu + impuls_mu == 0) {
            ...
            if ((masy == 2) || (masy == 3)) {// перегон
                ...
                if (fimpuls_kzm == -1) { // рисуем фоном ?
                        ...
                        if (ColorNum < 4) clr = PerSvetColors[ColorNum];
                        if (fimpuls_plus==1) clrP=KRA_SER;
                        if (fimpuls_neispr2==1) clrP=KRA_SER;
                        if ((CurrentPicture != BG) && (fimpuls_jelt==1) && (fimpuls_busi==0)&&(fimpuls_kzm==0)) clr= GELT;
                }
            } else

            if ((masy == 4) || (mas == 100)) { // Людочкин
                int cc = fimpuls_busi * 4 + fimpuls_kzm * 2 + fimpuls_kmu;
                if (cc >= 8) clr = BIRUZOVIJ; else if (cc < 0) clr = RED; else
                    clr = SvetStateColors100[cc];
                if (fimpuls_mnus) clrP = ZEL;
                // я не знаю зачем исключали при горящем!
                if ((fimpuls_plus == 1) && (fimpuls_busi == 0)) clr = COLOR_PSV;
                if ((fimpuls_plus == 1) && (fimpuls_busi == 1)) clr = ZEL_MIG;
            } else

                if ((masy == 5)) { // Приглас
                    int cc = fimpuls_busi + fimpuls_kzm * 2;
                    if (cc >= 4) clr = BIRUZOVIJ; else if (cc < 0) clr = RED; else
                        clr = SvetStateColors101[cc];

                    if (fimpuls_mnus) clrP = ZEL;
                    if ((fimpuls_plus == 1) && (fimpuls_busi == 0)) clr = COLOR_PSV;
                    if ((fimpuls_plus == 1) && (fimpuls_busi == 1)) clr = ZEL_MIG;
                    if (clr==FON) clrP=FON1;
                } else
                    ...
                    if ((masy == 6)) { // Ант ехтра
                        ...
                        if (cc<4) clrP = SvetPColors009_AV_n[cc];

                        // если нет неисправности проверяем соотв ко
                        if (clrP == C_D){
                           if ((fimpuls_kmu==1)&&(impuls_mu!=0)&&(fimpuls_mu==0)) clrP = KRA_CHERN;
                           if (fimpuls_neispr2==1) clrP = KRA_CHERN;
                        }
                        ...
                        if ((cc >= 8) || (fimpuls_busi == 33)) clr = BIRUZOVIJ; else
                                                               clr = SvetBColors009[cc];
                        if ((!bZAKRKRA)&&(clr==KRA)) clr = FON;
                        ...
                        if ((impuls_busi%1000>960-22)||(f(impuls_busi+0)==33)){
                                clr=BIRUZOVIJ ;clrB2=BIRUZOVIJ;
                        }
                        int clrB_BG=FON;
                        if (impuls_busi>=1000){
                                //if (f(impuls_busi-1 )==1) { clr=FON;                            clrB2=FON;}  //  Нет информации (НС)

                                if (f(impuls_busi+1 )==1) { clr=GELT;      clrB_BG=COLOR_SVZ;   clrB2=GELT;} //  Два желтых (НС)
                                if (f(impuls_busi+2 )==1) { clr=GELT;      clrB_BG=COLOR_SVZ;   clrB2=FON;} //  Желтый (НС)
                                if (f(impuls_busi+3 )==1) { clr=GELT_SER;  clrB_BG=COLOR_SVZ;   clrB2=GELT;} //  Желтый мигающий + желтый (НС)
                                if (f(impuls_busi+4 )==1) { clr=COLOR_SVZ; clrB_BG=COLOR_SVZ;   clrB2=FON;} //Зеленый (НС)
                                if (f(impuls_busi+5 )==1) { clr=BEL_SER;   clrB_BG=clr;         clrB2=KRA;} //  Белый мигающий + красный (пригласительный) (НС)
                                if (f(impuls_busi+6 )==1) { clr=COLOR_SVZ; clrB_BG=COLOR_SVZ;   clrB2=GELT;}//    Зеленый + желтый (НС)
                                if (f(impuls_busi+7 )==1) { clr=GELT_SER;  clrB_BG=COLOR_SVZ;   clrB2=FON;}//      Желтый мигающий (НС)
                                if (f(impuls_busi+8 )==1) { clr=TXT;       clrB_BG=TXT;         clrB2=FON;}//      Белый (НС)
                                if (f(impuls_busi+9) ==1) { clr=TXT;       clrB_BG=TXT;         clrB2=GELT;}//      Белый + желтый (НС)
                                if (f(impuls_busi+10)==1) { clr=TXT;       clrB_BG=TXT;         clrB2=GELT_SER;}//      Белый + желтый мигающий (НС)
                                if (f(impuls_busi+11)==1) { clr=TXT;       clrB_BG=TXT;         clrB2=COLOR_SVZ;}//      Белый + зеленый (НС)
                                if (f(impuls_busi+12)==1) { clr=TXT;       clrB_BG=TXT;         clrB2=GELT;}//      Белый + желтый (НС)
                                if (f(impuls_busi+13)==1) { clr=TXT;       clrB_BG=TXT;         clrB2=GELT_SER;}//      Белый + желтый мигающий (НС)
                                //if (f(impuls_busi+14)==1) { clrB=FON;   clrB2=FON;}//      Сигнал не блокирован (НС)
                                if (f(impuls_busi+15)==1) { fimpuls_block=1;}//      Сигнал блокирован (НС)
                                ...
                                if (f(impuls_busi+21)==1) { clrP=COLOR_SVZ;   }//      Автодействие включено (НС)
                                ...
                                if (f(impuls_busi+0 )==1) { clr=KRA;                            clrB2=FON;}  //  Красный (НС)

                                if (CurrentPicture == BG) clr=clrB_BG;
                        }
                        ...
                        if (impuls_busi>=1000){
                                if (f(impuls_busi-1 )==1) { clr=FON;  }  //  Нет информации (НС)

                                if (f(impuls_busi+1 )==1) { clr=FON;  }  //  Погасший (М2)
                                if (f(impuls_busi+2 )==1) { clr=TXT;  }  //  Разрешающее показание (М2)
                                if (f(impuls_busi+3 )==1) { clr=FON;  }  //  Синий (М2)
                                if (f(impuls_busi+4 )==1) { clr=TXT;  }  //  Два белых огня (М2)
                                //if (f(impuls_busi+5 )==1) { clrB=FON;  }  //  Не блокирован (М2)
                                if (f(impuls_busi+6 )==1) { fimpuls_block=1;  }  //  Блокирован (М2)
                                ...
                                if (f(impuls_busi+0 )==1) { clr=FON;  }  //  Запрещающее показание (М2)
    ...
    if ((_clrP != clrP) || (_clr != clr)) StateChanged = true;
    _clrP = clrP; _clr = clr;


    // если мигающий цвет превентивно рисуем ! (что такое 'превентивно' ?)
    if ((IsClrMig(clr)) ||
            (IsClrMig(clrP))) StateChanged = true;
...
int Svet_0::IsAlarmState()
{
    ...
    if ((bGorl) && (impuls_busi == 0)) {
        ...
        if (impuls_busi + impuls_plus + impuls_mnus + impuls_kzm + impuls_kmu + impuls_mu == 0) {
            ...
            // разбираем по типам
            if ((masy == 4) || (mas == 100)) { // Людочкин
                if (fimpuls_plus == 1) b = impuls_plus;
            } else if ((masy == 5)) { // Приглас
                if (fimpuls_plus == 1) b = impuls_plus;
            } else if ((masy == 6)) { // Ант ехтра
                if (fimpuls_plus == 1) b = impuls_plus;
                if ((fimpuls_kzm == 1) && (fimpuls_busi == 1)) b = impuls_kzm;
                if (fimpuls_mu == 1) b = impuls_mu;
            } else if ((masy == 7)) { // Люд ехтра
                if (fimpuls_plus == 1) b = impuls_plus;
            } else if ((masy == 8)) { // Люд ехтра
                if (fimpuls_plus == 1) b = impuls_plus;
                if (fimpuls_kmu == 1) b = impuls_kmu;
            } else if ((masy == 9)) { // ебилог
                if (fimpuls_plus == 1) b = impuls_plus;
            }else if ((masy == 0) && (mas < 100)) { // обычный
                if (fimpuls_plus == 1) b = impuls_plus;
...
bool  Svet_0::Show1()
{
    ...
    if ((masy == 1)) {
        if ((fimpuls_block==1)&&(CurrentPicture == LT))   ShowSvetBLOCK(xx,yy,sh,FON1,KRA);
    ...
    if (((masy == 2) || (masy == 3))) {
        ...
        */
        ...
        if (Razmer[CurrentPicture] == 0) sh = sh_y / 2;
        if (bNiz){
                 xx = (xx - _X_) / MUL_X;
                 yy = (yy - _Y_) / MUL_Y;
                 yy += DELTA_Y;
                 xx += DELTA_X;
                 if (masy == 2)yy = yy + MUL_Y - sh_y - 1; else
                 if (masy == 3)yy = yy + MUL_Y + sh_y + 1;
        } else {
                if (masy == 2)yy = yy + MUL_Y - sh_y/2 - sh - sh; else
                if (masy == 3)yy = yy - MUL_Y + sh_y/2 + sh + sh;
        }
        if (GetType() == 4) len = sh_y;
        if (GetType() == 3) len = -sh_y;


        //ShowPerSvet(xx, yy, GetType(), bz, fimpuls_plus, fimpuls_kzm, mas, masy, name,fimpuls_block);
        //return false;
        if ((fimpuls_block==1)&&(CurrentPicture == BG))   ShowSvetBLOCK(xx,yy,sh,FON1,KRA);
    }


    // не рисовать у людочкиных при неактивном му на участке
    if ((mas >= 100) && (fimpuls_plus != 1)) {
        if ((impuls_mu != 0) && (CurrentPicture == BG) && (fimpuls_mu != 1)) return false;
    }
...
void  Svet_R::Show()
{
    ...
    if ((MOD == ED)&&(impuls_yach!=0))  ShowYacheyka(xx-sh,yy+(yach_sdvig+1)*MUL_Y,C_D,FON,sh_y);
    // рисуем
    if (fimpuls_yach==1)
    ...
    if (AO->bPodgonSvet) {
        ...
        if (rr < 0)
                if (masx2 == 1)/*((name[1]=='¤')||(name[1]=='„'))*/ yy = yy - rr; else yy = yy + rr;
    }
    ...
    setfillstyle(1, clr);
    if ((name[1] == 'Ж') && (clr == TXT))  setfillstyle(1, GELT);
    if ((name[1] == 'К') && (clr == TXT))  setfillstyle(1, KRA);


    _SetText(F_DEFAULT, LEFT_TEXT, CENTER_TEXT);


    if ((M) && (Stek == 0)) xx = xx + MUL_X - sh * 2 - 1;

    if (ZagradSv == 1)
        ShowRombik(xx, yy, sh); else {
        fillellipse(xx, yy, sh, sh);
        if ((masy == 10)&&(CurrentPicture == LT)) {setfillstyle(1, clrB2);fillellipse(xx-2*sh, yy, sh, sh);}
        }
    ...
    if (((MUL_Y > AO->_NAZVA_)||(CurrentPicture == LT)) && (!M)) {
        ...
        int mm = mas; if (mm > 100)mm -= 100;
        switch (mm) {
        case 1:  _SetTextSize(-sh*2);
            OutTextXY(xx + len + 4, yy, name);
            _SetTextSize(1);
            break;
        case 2:  break;
        default : OutTextXY(xx+len+4, yy, name);
        }
    }
    _SetText(F_DEFAULT, LEFT_TEXT, BOTTOM_TEXT);
...
void  Svet_L:: Show()
{
    ...
    if ((MOD == ED)&&(impuls_yach!=0)) ShowYacheyka(xx+sh-sh_y,yy-(yach_sdvig+1)*MUL_Y,C_D,FON,sh_y);
    // рисуем
    if (fimpuls_yach==1)
    ...
    if (((MUL_Y > AO->_NAZVA_)||(CurrentPicture == LT)) && (!M)) {
        ...
        if ((MUL_Y < (AO->_NAZVA_)) && (!M)) _SetText(F_LITT, RIGHT_TEXT, CENTER_TEXT);
        if ((MUL_Y > (AO->_NAZVA_ + 2)) && (!M)) _SetText(F_LITT_B, RIGHT_TEXT, CENTER_TEXT);
        ...
        case 1:  _SetTextSize(-sh*2);
            ...
            break;
        case 2:  break;
        default : OutTextXY(xx+len-1, yy, name);
        }
    }
    _SetText(F_DEFAULT, LEFT_TEXT, BOTTOM_TEXT);
}
