...
void KT(int x, int y, int len, int sw, int ll2)
{
  ...
  kr[4] = x + len_4 - S(len);
  kr[5] = y1 - sh_y;
  kr[6] = x1 - len_4 - S(len) + S3(len);
  kr[7] = y;
  kr[8] = x + len2;
  kr[9] = y;
  kr[10] = x + len2;
  kr[11] = y + sh_y;
  kr[12] = x1 - len_4 + S(len);
  kr[13] = y + sh_y;
  kr[14] = x + len_4 + S(len) - S3(len);
  kr[15] = y1;
  kr[16] = x3;
  kr[17] = y1;
}
...
void _fillpoly_(int sw_tip, int kr[], int dr, bool bShowIsNormalMnus, int clr, int clrfill)
{
  ...
  if (bShowIsNormalMnus) {
    ...
    setcolor(C_D);
    linec2(kr[h] + 1 * n, kr[h + 1], kr[h + 2] + 1 * n, kr[h + 3], sh_y + 3, s, sh_y * 2.5 + 3, 0,
           0);
    linec2(kr[h] + 2 * n, kr[h + 1], kr[h + 2] + 2 * n, kr[h + 3], sh_y + 3, s, sh_y * 2.5 + 3, 0,
           0);
    linec2(kr[h] + 3 * n, kr[h + 1], kr[h + 2] + 3 * n, kr[h + 3], sh_y + 3, s, sh_y * 2.5 + 3, 0,
           0);
    linec2(kr[h] + 4 * n, kr[h + 1], kr[h + 2] + 4 * n, kr[h + 3], sh_y + 3, s, sh_y * 2.5 + 3, 0,
           0);
  }
                     ...
                     bool bShowPol)
{
  ...
  if (bShowIsNormalPlus) {
    int x1, x2;
    if (sd == 0) {
      x1 = kr[t1 * 2] - (shi * 2) * (1 - sd) + 1 * sd;
      x2 = kr[t1 * 2] - (shi * 3) * (1 - sd) + 1 * sd;
    } else {
      x1 = kr[t1 * 2] + (shi * 2) * sd - 1 * (1 - sd);
      x2 = kr[t1 * 2] + (shi * 3) * sd - 1 * (1 - sd);
    }
