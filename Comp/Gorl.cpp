...
void Gorl::usOpredKSP()
{
    ...
    for (int i = 0; i < gorl[0]->Col_Str; i++) {
        for (int j = 0; j < Size_G; j++) {
            clr0[j] = 0; itis[j] = D;
            if (gorl[j] == NULL) continue;
            if ((nom[j] = gorl[j]->_Strel[i]) == NULL)continue;/*���� ��� �������*/
            if (gorl[0]->bit_ksp == 1) {
    ...
    for (int i = 0; i < gorl[0]->Col_Mar; i++) {
        ...
        for (int j = 0; j < Size_G; j++) {
            ...
            if ((gorl[j]->Sign[i].name != 0) &&
                ...
                if (clr_svt[i] != Clr_SetC(1, gorl[j]->Sign[i].name)) clr_svt[i] = clr1; //??? c ���������
            }
        }
    }
...
void Gorl::GetPropMap(TPropMap &m)
{
    ...
    m.put("��Col_Mar", gorl[0]->Col_Mar);
    m.put("��Col_Str", gorl[0]->Col_Str);
    m.put("��bit_m", gorl[0]->bit_m);
    m.put("��bit_ksp", gorl[0]->bit_ksp);
    m.put("��bit_kzm", gorl[0]->bit_kzm);
    m.put("��nom_fiz", gorl[0]->nom_fiz);
    m.putEx("��impuls_ksp", gorl[0]->impuls_ksp  , GetRealImp(gorl[0]->impuls_ksp));
    m.putEx("��impuls_kzm", gorl[0]->impuls_kzm  , GetRealImp(gorl[0]->impuls_kzm));
    m.putEx("��impuls_ks", gorl[0]->impuls_ks    , GetRealImp(gorl[0]->impuls_ks));
    m.putEx("��impuls_ks_i", gorl[0]->impuls_ks_i, GetRealImp(gorl[0]->impuls_ks_i));

    m.put("��Sign_i_name", gorl[0]->Sign_i.name);
...
void Gorl::SetPropMap(TPropMap &m)
{
    ...
    strncpy(gorl[0]->name, m.get("�����").c_str(), 6);
    int cm = m.geti("��Col_Mar");
    int cs = m.geti("��Col_Str");
    ...
    // ��� ��������� �������� - �����������
    gorl[0]->bit_m             = m.geti("��bit_m");
    gorl[0]->bit_ksp           = m.geti("��bit_ksp");
    gorl[0]->bit_kzm           = m.geti("��bit_kzm");
    gorl[0]->nom_fiz           = m.geti("��nom_fiz");
    gorl[0]->impuls_ksp        = m.geti("��impuls_ksp");
    gorl[0]->impuls_kzm        = m.geti("��impuls_kzm");
    gorl[0]->impuls_ks         = m.geti("��impuls_ks");
    gorl[0]->impuls_ks_i       = m.geti("��impuls_ks_i");

    strncpy(gorl[0]->Sign_i.name, m.get("��Sign_i_name").c_str(), 6);
