...
int Test_KSP(int j, int* Flag_KSP, int* mem_KSP, int C_s)
{
  ...
  for (int i = 0; i < C_s; i++) {
    if (
        (Flag_KSP[i] != 0) &&
        (Flag_KSP[i] == mem_KSP[j]))
      acc = 1; //подняли флаг занятия по маршруту
  }
  if (acc)
    return 1;
  return 0;
  ...
  for (int i = 0; i < gorl[0]->Col_Str; i++) {
    for (int j = 0; j < Size_G; j++) {
      ...
      /*   if(marsh[j]>=gorl[0]->Col_Mar){ clr0[j]=LIN;itis[j]=D; if((gorl[0]->bit_m)==2)   itis[j]=State[i];  } */
        ...
        case 1:
          if (f(gorl[j]->impuls_kzm) == 1)
            clr0[j] = COLOR_SET;
          if (f(gorl[j]->impuls_kzm) == 33)
            clr0[j] = BIRUZOVIJ;
          break;
        case 2:
          if (f(nom[j]->impuls_kzm) == 1)
            clr0[j] = COLOR_SET;
          if (f(nom[j]->impuls_kzm) == 33)
            clr0[j] = BIRUZOVIJ;
          break;
        }
        /*--КСП----*/
        ...
        case 1:
          if (f(gorl[j]->impuls_ksp) == 1)
            clr0[j] = COLOR_B;
          if (f(gorl[j]->impuls_ksp) == 33)
            clr0[j] = BIRUZOVIJ;
          break;
        case 2:
          if (f(nom[j]->impuls_busi) == 1)
            clr0[j] = COLOR_B;
          if (f(nom[j]->impuls_busi) == 33)
            clr0[j] = BIRUZOVIJ;
          break;
        }
      } /* конец else */
      if ((marsh_y[j] < gorl[0]->Col_Mar) && (marsh_y[j] >= 0))
        ism[j] = gorl[j]->Matr[marsh_y[j]][i];
      else
        ism[j] = D;
      if (marsh_u[j] != -1)
        isu[j] = gorl[j]->Matr[marsh_u[j]][i];
      else
        isu[j] = D;
      ...
      for (marsh[j] = 0; marsh[j] < gorl[0]->Col_Mar; marsh[j]++) {
        if (f(gorl[j]->impuls_km[marsh[j]]) == 1) {
          if (priznak_2_marsh == -1) {
            ...
            priznak_2_marsh = gorl[0]->Col_Mar + 1;
            break;
          }
        }
      }
      if (priznak_2_marsh != -1)
        marsh[j] = priznak_2_marsh;
      for (marsh_y[j] = 0; marsh_y[j] < gorl[0]->Col_Mar; marsh_y[j]++)
        if (f(gorl[j]->impuls_m[marsh_y[j]]) == 1)
          break;
      /*---------------------
      ...
      for (marsh[j] = 0; marsh[j] < gorl[0]->Col_Mar; marsh[j]++) {
        ...
        for (i = 0; i < gorl[0]->Col_Str; i++) {
          ...
          if (!((gorl[j]->Matr[marsh[j]][i] == D) ||
            ...
            if (State[i] != gorl[j]->Matr[marsh[j]][i])
              break;
          } else /*пропустить пустую запись */
            Count_empty++;
        }
...
void Gorl::usOutStrel()
{
  ...
  for (int j = 0; j < Size_G; j++) {
    if (itis[j] == D)
      is_D++;
    if ((itis[j] == P) || (itis[j] == H))
      is_P++;
    if (itis[j] == M)
      is_M++;
    if (itis[j] == W)
      is_W++;
    if (clr1 < Clr_lev(clr0[j]))
      clr1 = Clr_lev(clr0[j]);
...
void Gorl::usOutSvet()
{
  ...
  for (int i = 0; i < gorl[0]->Col_Mar; i++) {
    ...
    for (int j = 0; j < Size_G; j++) {
      ...
      if ((gorl[j]->Sign[i].name != 0) &&
        ...
        if (clr_svt[i] != Clr_SetC(1, gorl[j]->Sign[i].name))
          clr_svt[i] = clr1; //??? c маневрами
      }
    }
  }
  ...
  for (int j = 0; j < Size_G; j++) {
    if (gorl[j] == NULL)
      continue;
    for (int i = 0; i < gorl[0]->Col_Mar; i++) {
      if ((sgn = (PElement)(gorl[j]->Sign[i].nomer)) == NULL)
        continue;
      else {
        sgn->clr = clr_svt[i]; //  << ВАУ !
        // sgn->Get();                      29.08.07
        // if(MEM.impuls_busi==0){
        if ((sgn->impuls_busi == 0) && (gorl[j]->impuls_ks == 0))
          sgn->clr = FON;
        sgn->Clear();
  ...
  for (int j = 0; j < Size_G; j++) {
    if (gorl[j] == NULL)
      continue;
    if ((sgn = (PElement)(gorl[j]->Sign_i.nomer)) == NULL)
      continue;
    else {
...
void Gorl::Load(FILE* file)
{
  if (MOD == ED) {
    ...
    if (gorl[0] == NULL)
      Qui("нет памяти для горловин", 1, 0);
...
void Gorl::Destroy(void)
{
  ...
  if (gorl[0]->Matr) {
    for (int i = 0; i < gorl[0]->Col_Mar; i++) {
      if (gorl[0]->Matr[i])
        delete[] gorl[0]->Matr[i];
    }
    delete[] gorl[0]->Matr;
  }
  if (gorl[0]->Strel_N)
    delete[] gorl[0]->Strel_N;
  if (gorl[0]->_Strel)
    delete[] gorl[0]->_Strel;
  if (gorl[0]->Sign)
    delete[] gorl[0]->Sign;
  if (gorl[0]->impuls_m)
    delete[] gorl[0]->impuls_m;
  if (gorl[0]->impuls_km)
    delete[] gorl[0]->impuls_km;
  if (gorl[0]->zona_M)
    delete[] gorl[0]->zona_M;
};
  ...
  try {
    ...
    for (int i = 0; i < gorl[0]->Col_Str; i++) {
      ...
      if (str == NULL)
        continue; /*если нет стрелки*/
      if (gorl[0]->bit_ksp == 1) {
        if (f(gorl[0]->impuls_ksp) == 1)
          BUSY = true;
      }
      if (gorl[0]->bit_ksp == 2) {
        if (f(str->impuls_busi) == 1)
          BUSY = true;
      }
      if (gorl[0]->bit_kzm == 1) {
        if (f(gorl[0]->impuls_kzm) != 1)
          KZM = false;
      }
      if (gorl[0]->bit_kzm == 2) {
        if (f(str->impuls_kzm) != 1)
          KZM = false;
      }
    }
    if (gorl[0]->Sign_i.nomer != NULL) {
      if (f(gorl[0]->Sign_i.nomer->impuls_busi) == 1)
        SvOpen0 = true;
    }
    if ((Marsh >= 0) && (Marsh < gorl[0]->Col_Mar)) {
      if (gorl[0]->Sign[Marsh].nomer != NULL) {
        if (f(gorl[0]->Sign[Marsh].nomer->impuls_busi) == 1)
          SvOpen1 = true;
      }
    }
  } catch (...) {
...
int Gorl::GetDoneMarsh()
{
  try {
    ...
    if ((Marsh >= 0) && (!BUSY) && (KZM) && (SvOpen0 || SvOpen1))
      return Marsh;
  } catch (...) {
  }
  return 0;
