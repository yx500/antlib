...
void Ways::GetPropMap(TPropMap &m)
{
    ...
    m.putEx("��impuls_svob", OldImpToNewStr(impuls_svob  , this), GetRealImp(impuls_svob),   OldImpToNewStr(0  , this));
    m.putEx("��impuls_IR", OldImpToNewStr(impuls_IR  , this), GetRealImp(impuls_IR),   OldImpToNewStr(0  , this));
    m.putEx("��impuls_predSEIR", OldImpToNewStr(impuls_predSEIR  , this), GetRealImp(impuls_predSEIR),   OldImpToNewStr(0  , this));
    String ss;
    for (int d = 0; d < 2; d++) {
        for (int i = 0; i < 2; i++) {
            ss = "��IDnext" + IntToStr(d) + IntToStr(i);
...
void Ways::SetPropMap(TPropMap &m)
{
    ...
    impuls_svob = NewStrToOldImp(m.get("��impuls_svob").c_str());
    impuls_IR = NewStrToOldImp(m.get("��impuls_IR").c_str());
    impuls_predSEIR = NewStrToOldImp(m.get("��impuls_predSEIR").c_str());
    String ss;
    for (int d = 0; d < 2; d++) {
        for (int i = 0; i < 2; i++) {
            ss = "��IDnext" + IntToStr(d) + IntToStr(i);
