...
)
{
  ...
  int PolLen = SH;
  if (M[3].y > fTd[2].y - PolLen)
    PolLen = fTd[2].y - M[3].y;
  ...
  PolLen = SH;
  if (M[5].y < fTd_[2].y + PolLen)
    PolLen = M[5].y - fTd_[2].y;
...
void TY_Strel::SetColors()
{
  ...
  default:
    clBP = fillclr;
    clBM = fillclr;
    clS1 = (prSTIK[0] == 0) ? clBP : clPP;
    break;
  case M:
    clBP = LIN;
    clBM = fillclr;
    clS1 = (prSTIK[0] == 0) ? clBM : clPP;
    break;
  case P:
    clBM = LIN;
    clBP = fillclr;
...
void TY_Strel::SetYPM()
{
  ...
  YP.prSTIK[0] = prSTIK[0];
  YP.prSTIK[1] = prSTIK[1];
  YP.prSTIK[2] = prSTIK[2];
}
...
void TY_Strel::Show()
{
  ...
  if (static_enabled) {
    _is = W;
    clr = static_clB;
    cbord1 = static_clP[CurrentPicture];
    cbord2 = static_clP[CurrentPicture];
    SetColors();
    _is = static_is;
  }
  // переделываем кму
...
void TY_Strel::ShowBLOCK()
{
  ...
  if (MOD == ED) {
    ...
    if ((fimpuls_block == 1) && (CurrentPicture == LT))
      PolyColor(KRA, P1, clr1, 4);
    if ((fimpuls_otkl_plus == 1) || (fimpuls_otkl_minus == 1))
      PolyColor(BIRUZOVIJ, P1, clr1, 4);
  }
}
