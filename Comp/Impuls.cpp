...
{
    LastValue = Value;
    if (FAbsNumber == 0) Value = 0; else if (FAbsNumber == -1) Value = 1; else if (FAbsNumber == -2) Value = 0; else

        //
        if (Parent != NULL) {
            Value = Parent->f(Chanel * 1000 + Number);
        } else {
            if (_f_ == NULL) Value = 33; else
...
void TAImpuls::FromString(const char *szSrc)
{
    ...
    if (stSrc == "") {
        Chanel = 0; Number = 0; Invers = 0; Value = 0; return;
    }
    if (stSrc == "-1") {
        Chanel = 0; Number = -1; Invers = 0; Value = 1; return;
    }
    if (stSrc == "-2") {
...
char * OldImpToNewStr(int impnumber, AComp * ac)
{
    ...
    if ((impnumber == -1) || (impnumber == -2)) {
        ...
        if (impnumber < 0) IMP.Invers = true;
...
int  NewStrToOldImp(const char * szStr)
{
    ...
    if (impnumber >= 1000) {
        if (IMP.Invers) impnumber = -impnumber;
    }
    return impnumber;
}
...
void __fastcall TAImpuls::SetAbsNumber(int value)
{
    ...
    if ((Parent != NULL) && (Parent->Stan() != NULL)) {
        ...
        //MessageDlg(IntToStr(value)+" �� ������ � ������ ������� �������", mtError  , TMsgDlgButtons() << mbOK , 0);
        Qui(" �� ������ � ������ ������� �������", 1, 1);
    } else {
        FAbsNumber = value;
        Number = value;
