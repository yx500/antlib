...
void Lamp::ShowX(int x, int y)
{
  ...
  if (AO->TypLamp == 32) {
    if ((AO->TypName == 3) && (MOD == RD) && (masy != 0) && (CurrentPicture == LT))
      return;
    //  ÏÎ ÎÑÒÓ ÊÐÓÆÎ×ÊÈ ÍÅ ÐÈÑÓÅÌ !!!
    if (MUL_Y < 16)
      _SetText(F_LITT, CENTER_TEXT, CENTER_TEXT);
    else
  ...
  if (AO->TypLamp == 33) {
    if (MUL_Y < 16)
      ...
      _SetText(ARIAL12, CENTER_TEXT, CENTER_TEXT);
    tw = textwidth(nNam) + 4;
    tw = (tw / MUL_X) * MUL_X + ((tw % MUL_X) && 1) * MUL_X - 2;
    th = textheight(nNam);
    th = (th / MUL_Y) * MUL_Y + ((th % MUL_Y) && 1) * MUL_Y - 2;
...
void Lamp::Set()
{
  ...
  {
    TLampMEM MM;
    memcpy(&MM, &MEM.name16, sizeof(TLampMEM)); // name[16]- äóðàöêèé ñäâèã
...
{
  // int typi;
  // if( !strcmp("Š€" ,name))  typi = 0;
  // if( !strcmp("Ž" ,name)) typi  = 1;
  // if( !strcmp("Ž—" ,name)) typi  = 2;
...
void Lam4::Show()
{
  ...
  if (name[0] != '$') {
    ...
    //
    // âîò òååïåðü ÿ òî÷íî íè÷åãî íå ïîíèìàþ
...
{
  uint32 tag;  //
  uint32 addr; //àäðåñ îò êîãî
  uint32 tick; //êîë ïðèøåäøèõ ðàêåòîâ
  uint32 time; //âðåìÿ ïðèøåäøåãî ïàêåòà
  uint32 msec; //âðåìÿ ïðèøåäøåãî ïàêåòà msec
};
#pragma pack(pop)
...
void SuperLamp2::SetStateColors()
{
  ...
  if (masy == 41) {
    ...
    if (AVStanInfo == NULL) {
      ...
      if (AVStanInfo->AV_DU) {
        clr = ZEL; // < >        < >
        if (AVStanInfo->z_cnt[nomer] == 1)
          clr = GELT;
        if (AVStanInfo->z_cnt[nomer] == 2)
          clr = KRA_GELT;
      }
    }
  } else if (masy < 0) {
  ...
  // åñëè ìèãàþùèé öâåò ïðåâåíòèâíî ðèñóåì ! (÷òî òàêîå 'ïðåâåíòèâíî' ?)
  if (IsClrMig(clr))
    StateChanged = true;
}
...
void ShowAUM(const char* name, int x, int y, int i1, int i1m, int i2, int i2m)
{
  ...
  if (CurrentPicture == BG) {
    if ((i1 != 1) && (i1m != 1) && (i2 != 1) && (i2m != 1))
      return;
  }
  int p = 0;
  int d = 0;
  int b = 0;
  if (strcmp("_ÀÓÌ×1_", name) == 0) {
    d = 0;
    p = 1;
  } else if (strcmp("_ÀÓÌ×2_", name) == 0) {
    d = 0;
    p = 2;
  } else if (strcmp("_ÀÓÌ×3_", name) == 0) {
    d = 0;
    p = 3;
  } else if (strcmp("_ÀÓÌÍ1_", name) == 0) {
    d = 1;
    p = 1;
  } else if (strcmp("_ÀÓÌÍ2_", name) == 0) {
    d = 1;
    p = 2;
  } else if (strcmp("_ÀÓÌÍ3_", name) == 0) {
    d = 1;
    p = 3;
  } else if (strcmp("_ÀÓÌ×1Á_", name) == 0) {
    b = 1;
    d = 0;
    p = 1;
  } else if (strcmp("_ÀÓÌ×2Á_", name) == 0) {
    b = 1;
    d = 0;
    p = 2;
  } else if (strcmp("_ÀÓÌ×3Á_", name) == 0) {
    b = 1;
    d = 0;
    p = 3;
  } else if (strcmp("_ÀÓÌÍ1Á_", name) == 0) {
    b = 1;
    d = 1;
    p = 1;
  } else if (strcmp("_ÀÓÌÍ2Á_", name) == 0) {
    b = 1;
    d = 1;
    p = 2;
  } else if (strcmp("_ÀÓÌÍ3Á_", name) == 0) {
    b = 1;
    d = 1;
    p = 3;
  }
  ...
  if (b) {
    if (i1 == 1)
      setfillstyle(SOLID_FILL, LIN);
    else
      setfillstyle(SOLID_FILL, FON1);
  }
  ...
  if (d == 0) {
    ...
    FP2[0] = FP1[0];
    FP2[0].x = FP1[0].x - tw - 1;
    FP2[1] = FP1[1];
    FP2[1].x = FP1[1].x - tw - 1;
    FP2[2] = FP1[2];
    FP2[2].x = FP1[2].x - tw - 1;
...
void SuperLamp2::Show()
{
  ...
  if ((MOD == ED)) {
    clr = 7;
    if ((impuls_busi == 0) && (impuls_plus == 0) && (impuls_mnus == 0) && (impuls_kzm == 0))
      clr = 9;
    timesec_dt = 99;
  }

  // íå ðèñóåì íà ó÷àñòêå áåç êìó
  ...
  if (MUL_Y < 16)
    r = 5;
  bool bkpp = false;
  if (strcmp("_ÊÏÏ_", name) == 0)
    bkpp = true;
  bool bzemlya = false;
  if (strcmp("_ÇÅÌËß_", name) == 0)
    bzemlya = true;
  ...
  case 0: {
    setcolor(clr);
    if (bkpp)
      ShowKPP(x, y, tw, th, clr);
    else if (bzemlya)
      ShowZEMLYA(x, y, tw, th, clr);
    else
      OutTextXY(x, y, name);
  };
      break;
  ...
  case 11: {
    ...
    if (bTime)
      ShowTime(x + tw / 2, y + th / 2 + 2, timesec_dt);
  };
      break;
  case 12: {
    setcolor(C_D);
    ...
    if (bTime)
      ShowTime(x + tw / 2, y1 + th + r + r + 1, timesec_dt);
  };
      break;
  case 21: {
    setcolor(C_D);
    ...
    if (bkpp)
      ShowKPP(x, y, tw, th, C_D);
    else if (bzemlya)
      ShowZEMLYA(x, y, tw, th, C_D);
    else
      OutTextXY(x, y, name);
  };
      break;
  ...
  case 22: {
    ...
    if (bkpp)
      ShowKPP(x + (tw) / 2, y1 + th / 2, tw, th, C_D);
    else if (bzemlya)
      ShowZEMLYA(x + (tw) / 2, y1 + th / 2, tw, th, C_D);
    else
  ...
  try {
    if (MOD == ED) {
      ...
      for (int i = 0; i < vComps.size(); i++) {
        ac = (AComp*)vComps[i];
        ac->UpdateState(); // ÍÅÊÐÀÑÈÂÎ ÒÎ ÊÀÊ!
        b = ac->IsAlarmState();
        ac->StateChanged = true; // ÓÆÀÑ
        if (b != 0)
          break;
      }
      if (b != impuls_busi)
