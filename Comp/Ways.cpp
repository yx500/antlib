...
void Way::GetMarkerPoint(int iMarker, int& aX, int& aY)
{
  ...
  if (iMarker == 2) {
    aX = X * MUL_X + _X_ + (WAY_X + mas) * MUL_X;
    aY = Y * MUL_Y + _Y_;
  }
}
...
void Way::Show()
{
  ...
  if (masy != 0) {
    ...
    Bs[3] = yy2 - sh_y / 2 - 1;
    Bs[4] = xx + len - 1;
    Bs[5] = yy2 - sh_y / 2 + sh_y + 1;
    Bs[6] = xx + 1;
    Bs[7] = yy1 - sh_y / 2 + sh_y + 1;
    Bs[8] = xx + 1;
    Bs[9] = yy1 - sh_y / 2 - 1;
    fillpoly(4, Bs);
  ...
  if (MyFishk != 0) {
    ...
    GE.PictWidthStan = sh_y * 4;
    GE.PictHeightStan = sh_y * 4;
    GE.PictHeightYch = sh_y * 4;
    GE.PictWidthYch = sh_y * 4;
    // GE.GridAllign=0;
    GE.TextFontNStan = 0;
    GE.TextFontNYch = 0;
...
{
  if ((impuls_busi != 0) && (impuls_plus != 0))
    return true;
  return false;
}
...
{
  Ways::GetPropMap(m);
  m.putEx("тсimpuls_kzm0", OldImpToNewStr(impuls_kzm0, this), GetRealImp(impuls_kzm0), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_kzm1", OldImpToNewStr(impuls_kzm1, this), GetRealImp(impuls_kzm1), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_svob2", OldImpToNewStr(impuls_svob2, this), GetRealImp(impuls_svob2), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_svob3", OldImpToNewStr(impuls_svob3, this), GetRealImp(impuls_svob3), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_zan4", OldImpToNewStr(impuls_zan4, this), GetRealImp(impuls_zan4), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_zan5", OldImpToNewStr(impuls_zan5, this), GetRealImp(impuls_zan5), OldImpToNewStr(0, this));
  m.putEx("тсimpuls_zan6", OldImpToNewStr(impuls_zan6, this), GetRealImp(impuls_zan6), OldImpToNewStr(0, this));
}
...
{
  int sost = 0;
  if ((fimp1 == 33) || (fimp2 == 33) || (fimp3 == 33) || (fimp4 == 33) || (fimp5 == 33))
    return 33;
  if (MOD == RD) {
    if (masy == 11) {
...
void Plot::UpdateState()
{
  ...
  fimpuls_kzm0 = rimpuls_kzm0;
  fimpuls_kzm1 = rimpuls_kzm1;
  fimpuls_svob2 = rimpuls_svob2;
  fimpuls_svob3 = rimpuls_svob3;
  fimpuls_zan4 = rimpuls_zan4;
  fimpuls_zan5 = rimpuls_zan5;
  fimpuls_zan6 = rimpuls_zan6;
  ...
  if (((masy == 2) || (masy == 3))) {
    ...
    if ((masy == 2) && (fimp3 <= 1))
      fimp3 = 1 - fimp3;
    if ((masy == 2) && (fimpuls_zan4 <= 1))
      fimpuls_zan4 = 1 - fimpuls_zan4;
    if ((masy == 2) && (fimpuls_zan5 <= 1))
      fimpuls_zan5 = 1 - fimpuls_zan5;
    if ((masy == 2) && (fimpuls_zan6 <= 1))
      fimpuls_zan6 = 1 - fimpuls_zan6;
    if ((fimp1 == 1) ||
  ...
  if (((masy == 4))) {
    ...
    if ((impuls_busi + impuls_kmu + impuls_mu != 0) && (impuls_svob + impuls_svob2 + impuls_svob3 != 0)) {
      if ((impuls_busi != 0) && (impuls_svob != 0) && ((fimpuls_busi + fimpuls_svob == 0) || (fimpuls_busi + fimpuls_svob == 2)))
        clrp = KRA_BEL;
      if ((impuls_kmu != 0) && (impuls_svob2 != 0) && ((fimpuls_kmu + fimpuls_svob2 == 0) || (fimpuls_kmu + fimpuls_svob2 == 2)))
        clrp = KRA_BEL;
      if ((impuls_mu != 0) && (impuls_svob3 != 0) && ((fimpuls_mu + fimpuls_svob3 == 0) || (fimpuls_mu + fimpuls_svob3 == 2)))
        clrp = KRA_BEL;
    }
  } else {
    // несовпадение занятости/свободности
  ...
  if (masy == 13) {
    ...
    if ((MOD == ED) && ((impuls_busi == 0) || (impuls_plus == 0) || (impuls_mnus == 0) || (impuls_kzm == 0) || (impuls_kmu == 0)))
      clr = FON;
  }
  if (masy == 14) {
    // Сигналы АБТЦ МШ  STA
    ...
    if ((MOD == ED) && ((impuls_busi == 0) || (impuls_plus == 0) || (fimpuls_mu == 0)))
      clr = FON;
  }
...
void Plot::Show()
{
  ...
  if ((MOD == ED) && ((masy == 2) || (masy == 3)) &&
      ((impuls_busi == 0) || (impuls_kmu == 0) || (impuls_mu == 0)))
    clr = FON;
  ...
  if ((CurrentPicture == BG) && (abs(len) <= 6) && (clrp == C_D) && (clr == COLOR_B))
    clrp = clr;
  setcolor(clrp);
  setfillstyle(1, clr);
  ...
  if ((CurrentPicture == BG) && (abs(len) <= 6))
    dd = 0;
  if ((clr == LIN) || (clr == FON)) {
    setfillstyle(1, FON);
    ...
    if (nom) {
      ...
      if ((impuls_mnus != 0)) { // выводим на уч-ке только активный иначе стираем
        if ((CurrentPicture != BG) || (fimpuls_mnus == 1) || (MOD == ED))
          ShowPerSvet(xsv, ysv, tsv, fimpuls_mnus, 0, 0, 0, masysv, nm, 0);
        else
          ShowPerSvet(xsv, ysv, tsv, fimpuls_mnus, 0, -1, 0, masysv, nm, 0);
      } else
        OutTextXY(xtext, ytext, nm);
      if (impuls_kzm != 0) {
        ...
        pc = C_D;
        if (fimpuls_kzm == 0)
          bc = GELT; // инверсия заведомо
        if ((CurrentPicture == BG) && (MOD != ED))
          pc = bc;
        setfillstyle(1, bc);
        setcolor(pc);
        int hh = sh_y;
        if (hh > MUL_X)
          hh = MUL_X; // шоб красиво
        if (tsv == 3)
          barx(xx + len / 2 - MUL_X / 2, Y1 - 2, xx + len / 2 + MUL_X / 2, Y1 - hh - 2);
        if (tsv == 4)
          barx(xx + len / 2 - MUL_X / 2, Y2 + 2, xx + len / 2 + MUL_X / 2, Y2 + hh + 2);
      }
    }
    if (nom2) {
...
void Bl_uh::Show()
{
  ...
  if ((AO->NumTch) &&
    ...
    if ((abs(nom) % 2 == nch_cod) && (nom))
      OutTextXY(x + ch__len, y - s + ss, nm);
    if ((abs(nom2) % 2 == nch_cod) && nom2)
      OutTextXY(x + nch_len, y - s + ss, nm2);
    if ((abs(nom) % 2 == ch__cod) && (nom))
      OutTextXY(x + nch_len, y + s + ss, nm);
    if ((abs(nom2) % 2 == ch__cod) && (nom2))
      OutTextXY(x + ch__len, y + s + ss, nm2);
    _SetText(F_DEFAULT, LEFT_TEXT, BOTTOM_TEXT);
  }
}
