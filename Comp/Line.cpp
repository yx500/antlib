...
void  Over::Show()
{
    ...
    if ((AO->TypPere == 32) || (AO->TypPere == 321)) return;
    xx = x - MUL_X / 3;
    int x2 = x + MUL_X / 3;
    //  ‡„…‘œ ‘„…‹€› ˆ‡Œ……ˆŸ „‹Ÿ 2•2
    int y2;
...
void  Nomer::Show()
{
    ...
    clr = LIN;
    if (MUL_Y < 2) return;
    if ((MUL_Y < AO->_NAZVA_) && (CurrentPicture == BG)) return;
    _SetText(F_LITT, CENTER_TEXT, CENTER_TEXT);
    setcolor(BLUE_m);
    switch (mas) {
    case 1:  _SetTextSize(-sh_y*2+2);
        OutTextXY(xx, yy + MUL_Y / 2, name);
        _SetTextSize(1);
        break;
    default : OutTextXY(xx, yy+MUL_Y/2, name);
    }
...
void  Stik::Show()
{
    ...
    if (name[1] == '*') sd2 = (sh - sh_y) / 2;
    if (CurrentPicture == LT) sd = 2; else sd = -1;
...
void __fastcall ShowArrow(int X1, int Y1, int X2, int Y2)
{
    ...
    double al;
    if ((X2 - X1) == 0) {
        if (Y1 > Y2)al = M_PI_2; else al = -M_PI_2;
    } else al = atan(1.*(Y1 - Y2) / (X2 - X1));
    if (X2 < X1)al += M_PI;
...
{
    switch (sd) {
    case 0: d1=0; d2=0; break;
    case 1: d1=dd1; d2=dd2; break;
    case 2: d1=-dd1; d2=-dd2; break;
    case 3: d1= dd1; d2=-dd2; break;
    case 4: d1=-dd1; d2= dd2; break;
    case 5: d1=-dd2; d2=-dd1; break;
    case 6: d1= dd2; d2=-dd1; break;
    case 7: d1=-dd2; d2= dd1; break;
    }
}
void __fastcall ShowShadow(int X1, int Y1, int X2, int Y2, int w, int clrShadow, int sd1, int sd2)
{
    ...
    switch (w) {
    case 0:
    case 1: dx[0]=1; dx[1]=1; dy[0]=1; dy[1]=1; break;
    case 2: dx[0]=1; dx[1]=2; dy[0]=1; dy[1]=2; break;
    case 3: dx[0]=2; dx[1]=2; dy[0]=2; dy[1]=2; break;
    case 4: dx[0]=2; dx[1]=3; dy[0]=2; dy[1]=3; break;
    case 5: dx[0]=3; dx[1]=3; dy[0]=3; dy[1]=3; break;
    default:dx[0]=w/2; dx[1]=w/2; dy[0]=w/2; dy[1]=w/2;
    }
...
void Lini2::Show()
{
    ...
    if (SpecTip == 1) {
        ...
        if (EndLine[0] == 1) clr1[3] = _Color;
        if (EndLine[1] == 1) clr1[1] = _Color;
        PolyColor(FON, P1, clr1, 4);
    } else if (SpecTip == 2) {
        int sd1 = 0;
        int sd2 = 0;
        if ((EndLine[0] >= 20) && (EndLine[0] < 30)) sd1 = EndLine[0] - 20;
        if ((EndLine[1] >= 20) && (EndLine[1] < 30)) sd2 = EndLine[1] - 20;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        case 17:
        case 18:
            if (i == 1) DrawA(xx[1-i], yy[1-i], EndLine[i]);
            break;
        }
    }
    setlinestyle(SOLID_LINE, 0, 1);
...
void Lini2::GetMarkerPoint(int iMarker,int &aX,int &aY)
{
    ...
    int w = mas * MUL_X  - SdvigX[0][CurrentPicture] + SdvigX[1][CurrentPicture];
    int h = masy * MUL_Y - SdvigY[0][CurrentPicture] + SdvigY[1][CurrentPicture];
    if (iMarker==1){aX=x;aY=y;}
    if (iMarker==2){aX=x+w;aY=y+h;}
}
