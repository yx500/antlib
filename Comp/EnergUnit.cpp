...
void SaveLampsLS(Station* Stan)
{
  ...
  for (int j = 0; j < Stan->POLE[WAYS]->GetArraySize(); j++) {
    ...
    if (cnt == _MaxLampsInPacket) {
      PrintErr(String("ѕереполнение буфера энергетиков по станции ") + String(Stan->Dat->filename));
...
void LoadLampsLS(Station* Stan)
{
  ...
  for (int j = 0; j < Stan->POLE[WAYS]->GetArraySize(); j++) {
    ...
    for (int cnt = 0; cnt < _MaxLampsInPacket; cnt++) {
      if (Stan->EnergStanLamps->Lamps[cnt].ObjID != eo->GetID())
        continue;
...
void TE_OBJ::UpdateState()
{
  ...
  if (type_obj == eng_lmp) {
    ...
    if (masy < 0) {
      ...
      clrplmp = FON1;
      if (tun_in_csv[0] + tun_in_csv[1] + tun_in_csv[2] + tun_in_csv[3] + tun_in_csv[4] + tun_in_csv[5] + tun_in_csv[6] + tun_in_csv[7] + tun_in_csv[8] + tun_in_csv[9] != 0)
        clrplmp = C_D;
      if ((st1 == 1) || (st2 == 1) || (st3 == 1) || (st4 == 1) || (CurrentPicture == LT))
        clrplmp = C_D;
...
void TE_OBJ::Show()
{
  ...
  if (CurrentPicture == BG)
    font_size = Prz[2] ? (uint8)(Prz[2]) : 0;
  if (font_size == 0) {
    if (type_obj == eng_lmp)
      font_size = _E_def_FontsSzLmp[CurrentPicture];
    else
  ...
  // подложки
  // если есть предупреждени€ то сначала предупреждение
  ...
  if (type_obj == eng_key) {
    if (tipVklOkc) {
      if (MOD != ED) {
        ...
        if (impuls_busi < 0) {
          if ((fimpuls_busi == 1) && (fimpuls_plus == 0))
            fimpuls_plus = 1;
          else if ((fimpuls_busi == 0) && (fimpuls_plus == 1))
            fimpuls_plus = 1;
          else if ((fimpuls_busi == 0) && (fimpuls_plus == 0))
            fimpuls_plus = 0;
          else if ((fimpuls_busi == 1) && (fimpuls_plus == 1))
            fimpuls_plus = 0;
        } else {
          if ((fimpuls_busi == 0) && (fimpuls_plus == 0))
            fimpuls_plus = 1;
          else if ((fimpuls_busi == 1) && (fimpuls_plus == 1))
            fimpuls_plus = 1;
          else if ((fimpuls_busi == 1) && (fimpuls_plus == 0))
            fimpuls_plus = 0;
          else if ((fimpuls_busi == 0) && (fimpuls_plus == 1))
            fimpuls_plus = 0;
        }
    ...
    // сначала пробуем готовую с состо€нием
    RECT Rect;
    Rect.left = xx;
    Rect.top = yy;
    Rect.right = xx + ww;
  ...
  if (type_obj == eng_lmp) {
    if (Picture.Length() > 0) {
      ...
      bool bTU = false;
      if ((type_tu == eng_tu_astmu) && (tun_in_csv[0] + tun_in_csv[1] + tun_in_csv[2] + tun_in_csv[3] + tun_in_csv[4] + tun_in_csv[5] + tun_in_csv[6] + tun_in_csv[7] + tun_in_csv[8] + tun_in_csv[9] != 0))
        bTU = true;
      if ((type_tu == eng_tu_luch) && (TypKp != 0))
        bTU = true;
      if (bTU) {
