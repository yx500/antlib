...
void AComp::GetPropMap(TPropMap &m)
{
    ...
    for (int i=0;i<3;i++){
        ...
        m.put(String("��Pad")+IntToStr(i+1)+String("xy"),    stxy , String(""));
        m.put("��Pad"+IntToStr(i+1)+"cmd",   Pad[i].cmd,    "" );
        m.put("��Pad"+IntToStr(i+1)+"param", Pad[i].param,  "" );
    }
...
void AComp::SetPropMap(TPropMap &m)
{
    ...
    for (int i=0;i<3;i++){
        String stxy=m.get("��Pad"+IntToStr(i+1)+"xy");
        if (stxy!=""){
                TParString ps("",";");
                ps.SetVal(stxy);
                for (int j=0;j<8;j++) Pad[i].xy[j]=ps.GetInt(j,0);
        }
        strncpy(Pad[i].cmd,  m.get("��Pad"+IntToStr(i+1)+"cmd").c_str(),sizeof(Pad[i].cmd));
        strncpy(Pad[i].param,m.get("��Pad"+IntToStr(i+1)+"param").c_str(),sizeof(Pad[i].param));
    }
