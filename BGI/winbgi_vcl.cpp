...
TColor BgiColor(unsigned int c)
{
  ...
  if (c < 256)
    clr = BgiDacColor(c);
  if ((GradientFactor != 1) && (c != FON))
    clr = GetGradientColor(clr, GradientFactor);
  return clr;
}
...
{
  ::SetRect(&TraceRect, MaxInt, MaxInt, -MaxInt, -MaxInt);
  return TraceRect;
}
...
void arc(int x, int y, int start_angle, int end_angle, int radius)
{
  ...
  tracecoord(ac.xstart, ac.ystart);
  tracecoord(ac.xend, ac.yend);
  if (dc == NULL)
    return;
...
{
  tracecoord(x - xradius, y);
  tracecoord(x + xradius, y);
  tracecoord(x, y - yradius);
  tracecoord(x, y + yradius);
  if (dc == NULL)
    return;
...
int textheight(const char* str)
{
  ...
  if ((dc == NULL) || (!UseNativeFontSize)) {
    ...
    if (h == 0)
      return ptm.tmHeight; // хоть какую высоту выдай
    return h;
  }
  GetTextMetrics(
...
void OutTextXY(int x, int y, const char* str)
{
  ...
  tracecoord(x - dx, y - dy);
  tracecoord(x - dx + tw, y - dy);
  tracecoord(x - dx, y - dy + th);
  tracecoord(x - dx + tw, y - dy + th);
