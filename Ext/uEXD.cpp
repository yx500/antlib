...
void __fastcall AddInDoublTNFilter(int imp)
{
    ...
    DoublTNCnt++;
    if (DoublTNCnt == 1024)DoublTNCnt = 0;
}
    ...
    try {
        ...
        if (Ver == 3) {
            ...
            if (DD->E.sz_struct>0/*sizeof(DD->E)*/) {
                ...
                if (DD->E.STATE_EXTNUM!=0) _TRINF.TrainNumber=DD->E.STATE_EXTNUM;
                if (DD->E.STATE_GAC_ACTIVE==0) _TRINF.TrainColor = 7;
...
TTrainInfo * __fastcall TEXD::TN_GetTN(int imp, const char * StanFN, const char * ChanelName, bool bUseDoublFilter)
{
    ...
    try {
        imp = abs(imp);
        if (imp < 1000) return NULL;
        TKrPack * KrPack;
        TKroshTrainInfo * KTI;
        if (bUseDoublFilter) {
            /* Проверяем на повтоные номера */
            if (FindInDoublTNFilter(imp)) return NULL;
        }
        int IMP = imp % 1000;
